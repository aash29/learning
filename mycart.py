...
class MyCartPoleContEnv(gym.Env):
    """
    Description:
        A pole is attached by an un-actuated joint to a cart, which moves along a frictionless track. The pendulum starts upright, and the goal is to prevent it from falling over by increasing and reducing the cart's velocity.
    Source:
    ...
    def __init__(self):
        ...
        self.action_space =  spaces.Box(-highF, highF, dtype=np.float32)
        self.observation_space = spaces.Box(-high, high, dtype=np.float32)
    ...
    def step(self, action):
        ...
        if not done:
            ...
            self.steps_beyond_done += 1
            reward = 0.0
        #print("state at the end of step:", np.array(self.state).transpose().squeeze() )
        return np.array(self.state).transpose().squeeze(), reward, done, {}
